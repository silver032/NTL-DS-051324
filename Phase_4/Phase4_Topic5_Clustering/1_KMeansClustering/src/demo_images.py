...
def ideal():
    ...
    x_neat, _ = datasets.make_blobs(n_samples=5000,
                                ...
                                random_state=2)
    #fit k-means
    km_neat = KMeans(n_clusters=3, random_state=2, n_init = 10).fit_predict(x_neat)
...
def messyOne():
    ...
    x_messy, labs3 = datasets.make_classification(n_samples=5000,
                                         n_features=10,
                                          ...
                                          shuffle=False,
                                          random_state=301)
    km_messy = KMeans(n_clusters=3, random_state=2, n_init = 10).fit_predict(x_messy)
    km_messy2 = KMeans(n_clusters=3, random_state=2, n_init = 10).fit(x_messy)
...
def messyTwo():
    ...
    km_unbal = KMeans(n_clusters=3, random_state=2, n_init = 10).fit(unbal)
    km_unbal_preds = KMeans(n_clusters=3, random_state=2, n_init = 10).fit_predict(unbal)
...
def messyThree():
    ...
    #Predict K-Means cluster membership
    km_unbal = KMeans(n_clusters=2, random_state=2, n_init = 10).fit(unbal)
    km_unbal_preds = KMeans(n_clusters=2, random_state=2, n_init = 10).fit_predict(unbal)
